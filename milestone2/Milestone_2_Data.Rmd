...
```{r}
  ggplot(nobel_prize_data, aes(x = year)) +
...
```{r}
ggplot(nobel_prize_data, aes(x = fct_infreq(category), fill = category)) +
  geom_bar() +
  coord_flip() +
...
```{r}
ggplot(nobel_prize_data, aes(x = age_at_award)) +
...
top_5_countries = nobel_prize_data %>% 
  ...
  unique() %>% 
  slice_max(order_by = total_award, n = 5)
...
ggplot(filtered_data) +
  geom_line(aes(year, total_award)) + 
